...
🆕 إخفاء رأس جداول Markdown الفارغة (جداول الرسم البياني النصي):
------------------------------------------------------------------
نص التحليل (Story Telling، راجع ai/prompt_builder.py::
_story_writing_rules قاعدة رقم 7) يُبني أحياناً رسماً بياناً نصياً
(عنصر/نسبة/شريط █░) داخل جدول Markdown برأس فارغ عمداً (بدون أسماء
أعمدة) — Markdown يتطلب صف رأس وصف فاصل '---' لصحة الصيغة حتى لو
كان الرأس بلا نص. بدون معالجة إضافية، Streamlit يُحوّل هذا الرأس
الفارغ إلى <thead> حقيقي يأخذ لون خلفية primary (من قاعدة
.stMarkdown thead أدناه) فيظهر كشريط ملوّن فارغ فوق الجدول. قاعدة
CSS جديدة (`.stMarkdown thead tr:has(th:empty):not(:has(th:not(:empty)))`)
...
def apply_theme_css(theme_key_or_settings="ocean_dark"):
    ...
    <style>
        ...
        /* ─────────────────────────────────────────────────────
           🆕 إخفاء رأس الجدول بالكامل عندما يكون فارغاً تماماً —
           حالة جداول "الرسم البياني النصي" في نص التحليل (Story
           Telling، راجع ai/prompt_builder.py::_story_writing_rules
           قاعدة رقم 7) حيث يُطلب من AI ترك خلايا الرأس فارغة (فقط
           صف الفاصل '---' المطلوب لصحة صيغة Markdown). بدون هذا،
           كان الرأس الفارغ يظهر كشريط ملوّن (لون primary) بلا نص —
           مربكاً بصرياً. الشرط
           :has(th:empty):not(:has(th:not(:empty))) يعني: كل خلايا
           هذا الصف فارغة معاً — جداول المقارنة العادية (برأس نصي
           فعلي، مثل قاعدة رقم 6 في الـ prompt) لا تتأثر إطلاقاً
           وتستمر بالظهور بلون primary كما هو.
           ───────────────────────────────────────────────────── */
        .stMarkdown thead tr:has(th:empty):not(:has(th:not(:empty))) {{
            display: none;
        }}
        .stMarkdown th {{
            color: #FFFFFF !important;
