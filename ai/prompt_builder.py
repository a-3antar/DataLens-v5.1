...
- أجب بـ SQL فقط بدون أي شرح"""
    ...
    def build_story(
        ...
        rules = (
            ...
            "بعدد متناسب (وليس رسماً فعليا لكن في سطر منفصل وواضح)، مثال:\n"
            "   الكمية المنتجة 25,000  █████████████░░░░░░░\t  (60%)\n"
            "   الأجزاء السليمه 14,000  ███████░░░░░░░░░░░░░\t  (56%)\n"
            "   ضعه داخل قسم كود (```) حتى تحافظ الأعمدة على محاذاتها، ولا "
            "تستخدمه إلا إذا أضاف وضوحاً حقيقياً و يجب ان يمثل رقم المستهدف 100% و الرقم المحقق نسبة منه.\n"
...
# ... داخل الكلاس PromptBuilder، بعد build_error_retry ...
...
"""    
    ...
    def build_story_multi(self, question: str, datasets: list, max_rows: int = 30) -> str:
        """
        نفس فلسفة build_story القديمة، لكن تستقبل datasets: قائمة
        ...
        rules = (
            ...
            "بصيغة Markdown منظَّمة (وليس فقرة نصية متصلة).\n\n"
            "القواعد الصارمة:\n"
            "1. اكتب تحليلاً نصياً (سرد قصصي/Storytelling) واحداً متكاملاً يشرح ما تدل عليه "
            "كل مجموعات البيانات أدناه معاً — قد تكون أكثر من مجموعة بيانات، كل واحدة تحت "
            "عنوان '### ' مستقل ضمن قسم DATA؛ اربط بينها في التحليل كلما كان ذلك مفيداً، "
            "وليس كوداً أو SQL أو JSON.\n"
            "2. استخدم فقط الأرقام والحقائق الموجودة فعلياً في البيانات أدناه — "
            "لا تخترع أرقاماً أو أسماء غير مذكورة.\n"
            "3. نظّم التحليل تحت عناوين فرعية بصيغة Markdown '### عنوان' "
