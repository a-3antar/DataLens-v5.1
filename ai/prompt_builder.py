...
- أجب بـ SQL فقط بدون أي شرح"""
    ...
    def build_story(
        ...
        🆕 القواعد أدناه تطلب الآن بنية Markdown صريحة (عناوين فرعية
        '### ' + نقاط '- ' + تخصيص أرقام مهمة بالـ '**bold**') بدل
        فقرة نصية متصلة — حتى يظهر السرد منظَّماً فعلياً في الواجهة
        (المحادثة ولوحات المعلومات) بدل نص خام متصل. طبقة العرض في
        ui/chat.py وcore/dashboard_cells/cells.py تعرض هذا النص عبر
        st.markdown مباشرة (بدون لفّه داخل <div> خام) حتى يُفسَّر
        الماركداون فعلياً — راجع تلك الملفات للتفاصيل.
        ...
        rules = (
            ...
            "4. تحت كل عنوان، استخدم نقاطاً (كل نقطة تبدأ بـ '- ' في بداية السطر) "
            "لعرض الحقائق والأرقام المنفصلة (أعلى/أدنى قيمة، اتجاه عام، تفاوت لافت)، "
            "بدل حشرها في فقرة متصلة. اجعل كل نقطة جملة واحدة واضحة.\n"
            "5. ضع الأرقام والقيم المهمة داخل '**تنسيق عريض**' لتبرز عن باقي النص.\n"
            "6. اختم بعنوان '### الخلاصة' يحتوي فقرة سردية قصيرة (2-3 جمل) "
            "أو ملاحظة عملية، وليس نقاطاً.\n"
            "7. لا تذكر أنك 'نموذج' أو تصف عملية التحليل نفسها — اكتب النتيجة مباشرة.\n"
            "8. لا تضع عنواناً رئيسياً بصيغة '#' أو '##' في بداية الرد — ابدأ "
